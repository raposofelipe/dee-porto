...
```{r}
comex <- readRDS("S:/CECAN/Felipe Raposo/NCM - Porto/Dados/[porto] comex_quasilimpo.rds")
```
...
```{r}
df_carga <- df_carga %>% 
  filter(!par_completo %in% readxl::read_excel("S:/CECAN/Felipe Raposo/NCM - Porto/Dados/[porto] COMBINACAO_FALTANTE.xlsx"), 
         !ncm4 %in% readxl::read_excel("S:/CECAN/Felipe Raposo/NCM - Porto/Dados/[porto] NCM_FALTANTE.xlsx"))
```
...
A divisão segue as combinações de natureza de carga
...
```{r}
# vai pro lixo no da ana
for (grp in 1:4) {
  df_carga %>% 
    filter(par_completo %in% list_nao_container[[grp]]) %>% 
    reframe(n = n(), .by = natureza_da_carga) %>% 
    mutate(porcentagem = n / sum(n)) %>% 
    print()
}
...
```{r}
# transformando em lista
list_ncm_combinacao <- ncm_combinacao %>% 
  group_by(combinacao) %>%
  summarise(ncms = list(ncm4)) %>%
  ungroup()

# o que vai virar lixos depois na comex
list_nao_ncm <- list_combinacao %>% 
  filter(!str_detect(combinacao, "Conteinerizada"))

list_nao_ncm <- setNames(as.list(list_nao_container$pares_completos), list_nao_container$combinacao)
```
...
comex <- comex %>% 
  filter(# limpando pares completos!
         !ncm4 %in% list_nao_ncm[[1]],
         !ncm4 %in% list_nao_ncm[[2]],
         !ncm4 %in% list_nao_ncm[[3]])
...
comex <- comex %>% 
  filter(# limpando pares completos!
         ...
         !par_completo %in% list_nao_container[[4]],
         selecionar != 0) %>% 
...
for (grp in 2:7) {
  ...
  comex_pred <- h2o::h2o.predict(object = ensemble_model, newdata = comex_pred)
...
Por uma questão de processamento, não conseguimos manter rodando uma base tão grande como a do grupo 1! Assim, optei por aleatorizar os pares, dar um número de 1 a 3 e quebrar a base
...
```{r}
tempo_grupo[[1]] <- vector("list", 5)
predito_grupo[[1]] <- vector("list", 5)
resultados_grupo[[1]] <- vector("list", 5)
```

```{r}
# base de treino
ensaio <- df_carga %>% 
    filter(par_completo %in% list_combinacao[[1]]) %>% 
    select(-ano,-ncm4, -par_simples) %>% 
    mutate(natureza_da_carga = str_to_lower(natureza_da_carga) %>% 
               stringi::stri_trans_general("Latin-ASCII") %>% 
               str_replace_all("[^\\w]", "_"), # natureza em classes
             )

set.seed(64)
ensaio <- ensaio %>% 
  sample_n(n()) %>% 
  mutate(index = rep(1:5, length.out = n()))
...
for (grp in 1:5) {
  ...
  ensaio_red <- ensaio %>% 
    filter(index == grp) %>% 
    select(-index) %>% 
    mutate(natureza_da_carga = as.factor(natureza_da_carga),
           ncm2 = as.factor(ncm2), # NCM em classes
           complexo_portuario = as.factor(complexo_portuario), 
           movimentacao = as.factor(movimentacao))
  ...
  set.seed(64)
  trainIndex <- caret::createDataPartition(ensaio_red$par_completo, p = 0.7, list = FALSE)
  
  train_data <- ensaio_red[trainIndex, -16]
  test_data <- ensaio_red[-trainIndex, -16]
  ...
  # random forest 
  random_comeca <- Sys.time()
  print(paste0("[1.", grp, "] Random Forest (início): ", random_comeca))
  ...
  random_acaba <- Sys.time()
  print(paste0("[1.", grp, "] Random Forest (fim): ", random_acaba))
  
  # naive Bayes
  naive_comeca <- Sys.time()
  print(paste0("[1.", grp, "] Naive Bayes (início): ", Sys.time()))
  ...
  naive_performance <- h2o::h2o.performance(model = naive_model, 
                                            newdata = test_data %>% h2o::as.h2o())
  naive_acaba <- Sys.time()
  print(paste0("[1.", grp, "] Naive Bayes (fim): ", naive_acaba))
  
  # glm
  glm_comeca <- Sys.time()
  print(paste0("[1.", grp, "] GLM (início): ", glm_comeca))
  ...
  glm_performance <- h2o::h2o.performance(model = glm_model,
                                          newdata = test_data %>% h2o::as.h2o())
  glm_acaba <- Sys.time()
  print(paste0("[1.", grp, "] GLM (fim): ", glm_acaba))
  
  # rede neural
  neural_comeca <- Sys.time()
  print(paste0("[1.", grp, "] Rede Neural (início): ", neural_comeca))
  ...
  neural_acaba <- Sys.time()
  print(paste0("[1.", grp, "] Rede Neural (fim): ", neural_acaba))
  
  # ensemble
  ensemble_comeca <- Sys.time()
  print(paste0("[1.", grp, "] Ensemble (início): ", ensemble_comeca))
  ...
  ensemble_acaba <- Sys.time()
  print(paste0("[1.", grp, "] Ensemble (fim): ", ensemble_acaba))
  ...
  # 3. Resultados
  resultados_grupo[[1]][[grp]] <- list("model" = ensemble_model, 
                                       "performance" = ensemble_performance)
  
  # tempo 
  tempo_grupo[[1]][[grp]] <- tibble(especificacao = c("todo",
                                                      "random", 
                                                      "naive", "glm", 
                                                      "neural",
                                                      "ensemble"), 
                               inicio = c(random_comeca,
                                          random_comeca,
                                          naive_comeca, 
                                          glm_comeca,
                                          neural_comeca, 
                                          ensemble_comeca), 
                               fim = c(ensemble_acaba, 
                                       random_acaba, 
                                       naive_acaba,
                                       glm_acaba, 
                                       neural_acaba,
  ...
  # predito
  predito_grupo[[1]][[grp]] <- comex_pred
  
  # apagando o que não é necessário ter salvo
  rm(list = ls()[!ls() %in% c("predito_grupo",
                              "resultados_grupo", # resultados: NCM grupo
                              "tempo_grupo", # tempo que demorou pra rodar
                              "list_combinacao", "list_nao_container",
                              "ensaio", "comex")]) # listas de pares
...
```{r}
save.image("S:/CECAN/Felipe Raposo/NCM - Porto/Resultados/[porto] classificacao_resultados")
```
